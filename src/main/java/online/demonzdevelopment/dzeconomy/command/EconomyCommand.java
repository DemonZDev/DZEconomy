...
public class EconomyCommand implements CommandExecutor, TabCompleter {
    ...
    private void handleStatus(CommandSender sender) {
        ...
        // Combat tagging status
        boolean combatTagEnabled = plugin.getCombatTagManager().isEnabled();
        int combatDuration = plugin.getCombatTagManager().getCombatDuration();
        int playersInCombat = plugin.getCombatTagManager().getPlayersInCombat();
        sender.sendMessage(ColorUtil.translate("&7Combat Tagging: " + (combatTagEnabled ? "&aEnabled &7(" + combatDuration + "s)" : "&cDisabled")));
        if (combatTagEnabled) {
            sender.sendMessage(ColorUtil.translate("&7Players in Combat: &c" + playersInCombat));
        }
    ...
    private void handleInfo(CommandSender sender) {
        ...
        sender.sendMessage(ColorUtil.translate("&7• &eCombat Tagging System (PVP/PvE)"));
